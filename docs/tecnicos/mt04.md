...
Actividad: <br>
Leer mínimo dos inputs (módulo sensor de distancias SRF05 / pulsador + fotoresistencias) y actuar mínimo dos outputs (servomotor + leds)
...
Primero hice el circuito en Tinkercad para realizar la simulación de que realmente la conexión funcionara.
...
const int bpulsar = 2;
const int led = 13; //indica que el led esta conectado al pin 13
int boton; // // se declara la variable boton

void setup(){
pinMode(bpulsar,INPUT); // se declara el pin 2 como entrada
pinMode(led, OUTPUT);   // se declara el pin 13 como salida 

}

void loop(){
  boton=digitalRead(bpulsar); // se asigna la variable "boton" el valor del pin 2
...
Armé las mismas conexiones en la placa de prueba y utilicé el código que me proponía tinkercad para arduino ide 



**Sensor de distancia con luces leds**

Materiales: Tres LEDs, una placa arduino uno, un cable de impresora, una computadora, cables para el montaje del circuito, placa de prueba, resistencia eléctrónica de .., un sensor de distancia. 

Primero hice el circuito en Tinkercad 

Pasos:

Primero hice el circuito en Tinkercad para realizar la simulación de que realmente la conexión funcionara.
FOTO PULSADOR 
![Fusion](../images/MT04/distancia.png)
